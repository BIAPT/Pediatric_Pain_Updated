function [result] = na_wpli_dsi24(recording, frequency_band, window_size, step_size, number_surrogate, p_value)
    %edited by Liz Teel- July 2021
    
    %the original na_wpli code calculates the wPLI overall and for several subregions. While this works perfectly for denser headsets
    %(like the  128 channel system), for less  dense headsets  (like the DSI-24) one or two missing channels or very small pieces of EEG data
    %(i.e. only one window) is enough to cause errors/break the code for some of the more specific subregions (i.e. left anterior middle, right posterior midline, etc.).
    %This na_wpli_dsi24 code removes some of these more specific subregions of analysis to prevent these errors. 
