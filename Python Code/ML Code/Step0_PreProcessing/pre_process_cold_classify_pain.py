...
def pre_process_acute_pain_baseline_vs_cold_pain(input_filename):
    ...
    # this is for the pain (0), so dropping the controls (1)
...
def pre_process_acute_pain_baseline_vs_cold_pain_subset(input_filename):
    """This function loads, reshapes and cleans the data frame so that it is amenable for machine learning
        Args:
            input_filename (string): This is the path to the dataset saved as a .csv file (DF_FILE_PATH in Step1_LoadCleanData.py)
        Returns:
    ...
    # Read the CSV
    df = input_filename # import dataset and call it "df"
